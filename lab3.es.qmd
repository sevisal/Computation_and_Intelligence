---
title: "Sesión de laboratorio 3: Problemas de Regresión"
format:
    html:
        toc: true
        toc-depth: 5
...
Puedes descargar el conjunto en formato CSV desde este enlace: https://raw.githubusercontent.com/ageron/handson-ml/master/datasets/housing/housing.csv (si lo abres en el navegador, haz clic derecho y selecciona "Guardar como..." para descargarlo). Una explicación de cada atributo está disponible en la documentación de scikit-learn: https://scikit-learn.org/stable/datasets/real_world.html#california-housing-dataset.
...
La idea de la regresión lineal es encontrar los coeficientes (pesos) para cada predictor que minimicen la suma de los cuadrados de las diferencias entre los valores observados y los predichos. Esto se conoce como el método de **Mínimos Cuadrados Ordinarios (OLS)** y tiene la siguiente formulación matemática:
$$
...
1. ¿Qué variables parecen tener el efecto más fuerte sobre los precios de la vivienda según los coeficientes de regresión?
2. ¿Podemos usar estos coeficientes para evaluar la importancia de las variables? ¿Por qué sí o por qué no?
3. ¿Observas coeficientes inusualmente grandes o inestables? ¿Qué podría indicar esto?
:::
...
En Orange puedes usar el widget `Test & Score` para evaluar el rendimiento de tu modelo de regresión. Este widget ofrece varias métricas, entre ellas:

- **Error cuadrático medio (MSE):** El promedio de los cuadrados de las diferencias entre los valores predichos y los reales. Penaliza más los errores grandes que el MAE, por lo que es sensible a valores atípicos. Se calcula como:
$$
MSE = \frac{1}{N} \sum_{n=1}^{N} (y_n - \hat{y}_n)^2
$$
donde $y_n$ es el valor real, $\hat{y}_n$ es el valor predicho y $N$ es el número de observaciones.
- **Raíz del error cuadrático medio (RMSE):** La raíz cuadrada del MSE, proporcionando una métrica de error en las mismas unidades de la variable objetivo. Se calcula como:
$$
RMSE = \sqrt{MSE} = \sqrt{\frac{1}{N} \sum_{n=1}^{N} (y_n - \hat{y}_n)^2}
$$
- **Error absoluto medio (MAE):** El promedio de las diferencias absolutas entre los valores predichos y los reales. Da una idea de cuánto se desvían las predicciones, en promedio. Se calcula como:
$$
MAE = \frac{1}{N} \sum_{n=1}^{N} |y_n - \hat{y}_n|
$$

- **Error porcentual absoluto medio (MAPE):** El promedio del porcentaje absoluto de la diferencia entre predicción y realidad. Es útil para entender el error en términos relativos, especialmente cuando la variable objetivo tiene un rango amplio. Se calcula como:
$$
MAPE = \frac{100\%}{N} \sum_{n=1}^{N} \left| \frac{y_n - \hat{y}_n}{y_n} \right|
$$
- **R-cuadrado ($R^2$):** La proporción de la varianza de la variable objetivo que es explicada por el modelo. Varía entre 0 y 1 (aunque puede ser negativo para modelos muy malos), siendo valores más altos indicativos de mejor ajuste. Se calcula como:
$$
...
1. ¿Qué métrica de evaluación crees que es más apropiada para este problema de regresión? ¿Por qué?
2. ¿Cómo afecta la estandarización de las características al rendimiento del modelo?
3. Según el diagrama de dispersión, ¿qué tan bien captura el modelo la relación entre las características y la variable objetivo?
:::

<!-- Solución
1. RMSE suele preferirse porque entrega el error en las mismas unidades que la variable objetivo y penaliza más los errores grandes que el MAE.
2. En teoría, la regresión lineal no es sensible al escalado de las características en cuanto a predicción; sin embargo, la estandarización puede mejorar la estabilidad numérica y hacer más interpretable los coeficientes.
3. El diagrama de dispersión debería mostrar una relación cercana a la diagonal si el modelo captura bien la relación, aunque pueden observarse desviaciones especialmente en valores extremos.
-->
...
Para ilustrar los efectos de la regularización, crearemos un conjunto de datos sintético con características repetidas que pueden inducir sobreajuste. Puedes descargar el conjunto con características repetidas de `MedInc` en el siguiente enlace: [Descargar conjunto con características redundantes](https://github.com/sevisal/Computation_and_Intelligence/raw/main/_resources/data/california_housing_data_rep.csv).
...
1. ¿Cómo cambian los coeficientes con distintos valores de $\lambda$ para Ridge y LASSO?
2. ¿Qué técnica de regularización parece funcionar mejor en este escenario? ¿Por qué?
3. ¿Cómo ayuda la regularización a mitigar el sobreajuste en este caso?
:::

<!-- Solución
1. A medida que $\lambda$ aumenta, los coeficientes en Ridge y LASSO tienden a reducirse hacia cero. LASSO puede llevar algunos coeficientes exactamente a cero, realizando selección de variables.
2. LASSO puede funcionar mejor en este escenario porque puede eliminar características redundantes poniendo sus coeficientes a cero, produciendo un modelo más simple.
3. La regularización penaliza coeficientes grandes, desalentando que el modelo ajuste ruido en los datos de entrenamiento y mejorando la generalización a datos no vistos.
-->
...
En esta sección trabajaremos con un escenario de alta dimensionalidad con características polinomiales. Puedes descargar el conjunto con características polinomiales desde: [Descargar conjunto polinomial](https://github.com/sevisal/Computation_and_Intelligence/raw/main/_resources/data/california_housing_data_poly.csv).
...
En Orange puedes usar el widget `Test & Score` con validación cruzada para evaluar el rendimiento de Ridge y LASSO con distintos valores de $\lambda. Orange no dispone de un sintonizador de hiperparámetros automático, por lo que deberás crear un widget `Linear Regression` por cada valor de $\lambda y conectarlos al `Test & Score`. Asegúrate de incluir el widget de `Preprocessing` con la opción `Standardize` y pasarlo también al `Test & Score` para que la estandarización se haga dentro de cada pliegue y evitar fuga de datos.
...
1. ¿Cuál es el valor óptimo de $\lambda$ para Ridge y LASSO según la validación cruzada?
2. ¿Cómo compara el rendimiento de los modelos finales con el modelo de regresión lineal sin regularización?
3. ¿Cómo ayuda la validación cruzada a seleccionar el parámetro de regularización y mejorar la generalización del modelo?
:::

<!-- Solución
1. El valor óptimo de $\lambda$ depende del conjunto de datos y los resultados de la validación cruzada; típicamente es el que minimiza el error medio de validación.
2. Los modelos finales con regularización deberían tener mejor rendimiento en el conjunto de prueba que el modelo sin regularización, al reducir el sobreajuste.
3. La validación cruzada proporciona una estimación más robusta del rendimiento al evaluar el modelo en múltiples particiones, ayudando a elegir un $\lambda$ que generalice bien.
-->
...
No hay una única solución correcta. Lo importante es que puedas explicar y justificar tus decisiones. Al final, guarda tu flujo de trabajo y escribe una breve reflexión describiendo cómo preparaste los datos, qué pasos consideraste innecesarios, qué objetivo elegiste para predecir y cómo afectó la regularización al rendimiento del modelo final.
